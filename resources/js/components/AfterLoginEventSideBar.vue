<template>
    <div class="login_flow">
        <div>
            <div class="right_card1 ">
                <div class="right_top p-2" style="display: block;">
                    ...
                    <label class="indi_btn"><a href="current-more-event" class="indi_btn"
                            style="margin-top: -28px;">View all</a></label>
                    <div id="carousel_inner_login_events1" class="carousel slide" data-bs-ride="carousel"
                        ref="carousel1">
                        <div class="carousel-inner">
                            <div v-for="(res, index) in currentEvent" :key="index"
                                :class="{ 'carousel-item': true, 'active': index === 0 }">
                                <p v-if="currentEvent.length <= 0">{{ this.message }} !</p>
                                ...
                                res.total_view }}</span>
                                                ...
                                                <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xxl-3 col-xl-3">
                                                    <div class=" text-center" style="margin-left: 20px;">
                                                        <a class="view_all_latestact" :href="`/sharepostcard?id=${btoa(res.id)}&type=${btoa(res.type)}`" target="_blank"  @click.prevent ="sendToWhatsApp( res.organization.title , `/sharepostcard?id=${btoa(res.id)}&type=${btoa(res.type)}`)" id="share">
        ...
        <div class="right_card2 mt-3">
            <div class="right_top p-2" style="display: block;">
                <label class="section_body p-1" style="font-weight: bold;">Upcoming Events</label>
                <label class="indi_btn"><a href="upcomming-more-event" class="indi_btn" style="margin-top: -28px;">View
                        all</a></label>
                <div id="carousel_inner_login_events2" class="carousel slide" data-bs-ride="carousel" ref="carousel">
                    <div class="carousel-inner">
                        <div v-for='(res, index) in upcomingEvent' :key="index"
                            :class="{ 'carousel-item': true, 'active': index === 0 }">
                            <div class="card h-9 p-2 card_getallnews">
                                <div class="ribbon-wrapper">
                                    <div class="card-badge-event">Events</div>
                                ...
                                <div class="row">
                                    <div class="col">
                                        <a class="view_all_latestact"  :href="`/sanstha-detail/${res.id}`">
                                            <img :src="`${res.organization ? res.organization[0] ? res.organization[0].original_url : 'images/20d42aa2-c51f-45bb-9cdf-05c6ed26f99d.png' : 'images/20d42aa2-c51f-45bb-9cdf-05c6ed26f99d.png'}`"
                                            :title="res.organization.title"  :alt="res.organization.title" class="getallimg img-fluid">
                                            <span class="d-inline-block text-truncate getallhead"
                                                :title="res.organization.title" alt="Organization Title">{{
                                res.organization.title }}</span>
                                    ...
                                    <div class="card-footer border-0 bg-white activity_card_footer">
                                        <div class="row row-cols-4">
                                            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xxl-3 col-xl-3">
                                                <div class="text-center" style="margin-left: -45px;">
                                                    <img :src="getImageUrl(res)"
                                                        @click="manageLike1(res, res.type, res.id, res.get_user_like == null ? is_like = 0 : res.get_user_like.is_like, res.total_like, index)"
                                                        class="footer_img" id="unLike_fav" alt="...">
                                                    <br><span class="icon_font">{{ res.total_like == null ? 0 :
                                res.total_like }}</span>
                                                </div>
                                            ...
                                            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xxl-3 col-xl-3">
                                                <div class="text-center" style="margin-left: -21px;">
                                                    <img :src="getSaveImageUrl(res)"
                                                        @click="manageSave1(res,res.type,res.id,res.get_user_save_post == null ? is_save=0 : res.get_user_save_post.is_save,index)"
                                                        class="footer_img" alt="...">
                                                    <br>
                                                    <span v-if="getSaveImageUrl(res) === '/images/ic_add_fav_box_24px.svg'" class="icon_font">Saved Post</span>
                                                    <span v-else class="icon_font">Save Post</span>
                        ...
                        <p v-if="upcomingEvent.length <= 0">{{ this.message }} </p>
                    </div>
                    <br><br>
                    <div class="carousel-indicators">
                        <button v-for="(res, index) in upcomingEvent" :key="index" type="button"
                            :data-bs-target="'#carousel_inner_login_events2'" :data-bs-slide-to="index"
                            :class="{ 'active': index === 0 }" aria-current="true" :aria-label="`Slide ${index + 1}`"
                            @click="goToSlideEvent(index)"></button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            user_name: [], currentEvent: [], upcomingEvent: [], add_like: '', save_img: '', message: '',
        };
    },
    methods: {
        ...
        goToSlide(index) {
            const carousel = new bootstrap.Carousel(this.$refs.carousel1);
            carousel.to(index);
        },
        goToSlideEvent(index) {
