<template>
    <section class="mt-5 pt-5 pb-5">
        <div class="join_volunteer_title text-center"> join us</div>
        <div class="card join_volunteer_card mx-3 mt-3 pb-3">
            <form class="px-4">
                ...
                <label for="bg_volunteer" class="fullname_volunteer_label form-label mt-3">Background</label>
                <textarea class="fullname_volunteer_input form-control" v-model="background" id="bg_volunteer"></textarea>
                <label for="help_volunteer" class="fullname_volunteer_label form-label mt-3">How you can help</label>
                <textarea class="fullname_volunteer_input form-control" v-model="how_you_can_help" id="help_volunteer"></textarea>
                <label for="write_volunteer" class="fullname_volunteer_label form-label mt-3">Write To Us</label>
                <textarea class="fullname_volunteer_input form-control" v-model="write_to_us" id="write_volunteer"></textarea>
                <div class="d-grid gap-2 d-md-block mt-3">
                    <button class="volunteer_btn" type="reset">Clear</button>
                    <button class="volunteer_btn_submit" @click.prevent="postJoinVolunteer()" type="submit">Submit</button>
                </div>
            </form>
        </div>
    </section>
...
export default {
    ...
    data() {
        return {
            info:'',name:'',nameErr:'',emailErr:'',mobileErr:'',feedbackErr:'',email:'',mobile:'',
            feedback:'',background:'',how_you_can_help:'',write_to_us:'',
        };
    },
    methods: {
        async postJoinVolunteer(){
            this.nameErr = '';
            this.mobileErr = '';
            this.emailErr = '';
            this.feedbackErr = '';
            ...
            if (this.nameErr || this.mobileErr || this.emailErr || this.feedbackErr) {
                ...
                const data = {
                    ...
                    how_you_can_help:this.how_you_can_help,
                    write_to_us:this.write_to_us
                }
                if(this.name != '' && this.email != '' && this.mobile != '' && this.feedback != ''){
                    this.nameErr = '';this.mobileErr = '';this.feedbackErr = '';this.emailErr='';
                    const response = await volunteerService.createVolunteer({...data})
                    if(response.status == 200 || 201){
                        this.name='';this.email='';this.mobile='';this.feedback='';
                        this.how_you_can_help='';this.write_to_us='';this.background='';
                        this.$toast.success('Thank you for showing interest for Volunteering');
                    }else{
