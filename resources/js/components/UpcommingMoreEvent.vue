<template>
    <section class="pt-5">
        <div class="container pt-5 mt-5">
            <div class="row g-4">
                <a href="sharvak-dashboard" class="view_all_latestact label_manage_sanstha mt-5 mt-xl-2 mt-md-3 mt-lg-2 mt-xxl-2"><i class="bi bi-arrow-left me-3"></i>Upcoming Events</a>
                <div v-if="upcommingEvent.length == 0">{{ this.message }}</div>
                <div class="col-xxl-3 col-xl-4 col-lg-4 col-md-6 col-sm-12 col-xs-12 col-xxs-12"
                    v-for='(res, index) in upcommingEvent' :key="index">
                        ...
                        <div class="row">
                            <div class="col">
                                <a class="view_all_latestact"  :href="`/sanstha-detail/${res.organization_id}`">
                                    <img :src="`${res.organization ? res.organization.logo[0] ? res.organization.logo[0].original_url : 'images/20d42aa2-c51f-45bb-9cdf-05c6ed26f99d.png' : 'images/20d42aa2-c51f-45bb-9cdf-05c6ed26f99d.png'}`"
                            ...
                            <img :src="`${res.banner ? res.banner[0] ? res.banner[0].original_url : 'images/20d42aa2-c51f-45bb-9cdf-05c6ed26f99d.png' : 'images/20d42aa2-c51f-45bb-9cdf-05c6ed26f99d.png'}`"
                                ...
                                <div class="row row-cols-4">
                                    ...
                                    <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xxl-3 col-xl-3">
                                        <div class=" text-center share_div_current">
                                            <a class="view_all_latestact" :href="`/sharepost?id=${btoa(res.id)}&type=${btoa('Events')}`" target="_blank"  @click.prevent="sendToWhatsApp(  res.organization.title , `/sharepost?id=${btoa(res.id)}&type=${btoa('Events')}`)" id="share">
            ...
            <infinite-loading v-if="this.message == ''" class="pb-5" @distance="1"
                @infinite="getupcommingEvent"></infinite-loading>
        </div>
                    ...
                    <div class="modal-body" style="padding-bottom: 0%;">
                        <div class="row">
                            <div class="col-md-12 col-lg-6 col-xl-6 col-xxl-6 col-sm-12 col-xs-12">
                                ...
                                <div class="swiper thumbnail thumbnailUpcoming">
                                    <div class="swiper-wrapper">
                                    ...
                                    <div class="next_div" v-if="img.length > 3" @click="nextSlideUpcoming"><i
                                            class="bi bi-chevron-right"></i></div>
                                    <div class="prev_div" v-if="img.length > 3" @click="prevSlideUpcoming"><i
...
<script>
import upcommingService from "../../services/upcommingevent.service";
...
import { initializeSliderUpcoming, initializeThumbnailUpcoming } from "../../utils/swiper";
...
export default {
    ...
    data() {
        return {
            message: '', page: 1, upcommingEvent: [], total_count: '', dash_sanstha: [], img: '', selectedImageIndex: 0, org_logo: '', org_title: '', total_count: '', about_id: '',
        };
    },
    methods: {
        formatTimeAgo(date) {
        ...
        async getupcommingEvent($state) {
            const response = await upcommingService.getUpcomming(this.page)
            if (response.status == 200 || 201) {
                if (response.data.length != 0) {
                    $.each(response.data, (key, value) => {
                        this.upcommingEvent.push(value);
                    });
                    $state.loaded();
        ...
        async getTypedSanstha() {
            ...
            if (response.status === 200 || 201) {
                ...
                console.log("Success",this.dash_sanstha.type)
        ...
        async manageLikeMore() {
            ...
            const newsIndex = this.upcommingEvent.findIndex(explore => explore.id === this.dash_sanstha.id);
            if (newsIndex !== -1) {
                this.$set(this.upcommingEvent[newsIndex], 'total_like', newLikeCount); // Update total_like
                if (this.upcommingEvent[newsIndex].get_user_likes) {
                    this.$set(this.upcommingEvent[newsIndex].get_user_like, 'is_like', newIsLike);
                } else {
                    this.$set(this.upcommingEvent[newsIndex], 'get_user_like', { is_like: newIsLike });
                }
            }
        ...
        async manageSaveMore() {
            ...
            const newsIndex = this.upcommingEvent.findIndex(explore => explore.id === this.dash_sanstha.id);
            if (newsIndex !== -1) {
                if (this.upcommingEvent[newsIndex].get_user_save_posts) {
                    this.$set(this.upcommingEvent[newsIndex].get_user_save_post, 'is_save', newIsSave);
                } else {
                    this.$set(this.upcommingEvent[newsIndex], 'get_user_save_post', { is_save: newIsSave });
                }
            }
        ...
        async manageFollowMore() {
            ...
            const newsIndex = this.upcommingEvent.findIndex(explore => explore.id === this.dash_sanstha.id);
            if (newsIndex !== -1) {
                if (this.upcommingEvent[newsIndex].get_user_org_follows) {
                    this.$set(this.upcommingEvent[newsIndex].get_user_org_follows, 'is_follow', newIsFollow);
                } else {
                    this.$set(this.upcommingEvent[newsIndex], 'get_user_org_follows', { is_follow: newIsFollow });
                }
            }
        ...
        nextSlideUpcoming() {
            const thumbnailUpcoming = document.querySelector('.thumbnailUpcoming').swiper;
            thumbnailUpcoming.slideNext();
        },
        prevSlideUpcoming() {
            const thumbnailUpcoming = document.querySelector('.thumbnailUpcoming').swiper;
            thumbnailUpcoming.slidePrev();
        },
        getDataById(resId) {
            this.about_id = resId
            if (this.about_id) {
        ...
        async manageLike(res, resType, resId, is_like, like_count, index) {
            ...
            const updatedItem = {
                ...this.upcommingEvent[index],
                get_user_like: { is_like: newIsLike },
                total_like: newLikeCount,
            };
            this.$set(this.upcommingEvent, index, updatedItem);
    ...
    mounted() {
        const thumbnailUpcoming= initializeThumbnailUpcoming();
        initializeSliderUpcoming(thumbnailUpcoming);
